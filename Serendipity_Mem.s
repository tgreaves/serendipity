...
*              A500  (1 Meg)           - Kickstart 1.3 (FAILURE!!!)
*
* KNOWN BUGS
* ==========
* 
* At least 1 megabyte of Chip ram required to run the production.
* (What about if modules are uncrunched??)
* Also ensure that modules are decrunched to CHIP RAM!
* 
* Awaiting Powerpacker docs to continue.
*
* Last updated 11th March 1994
*
* Last changes:		Section directives included to save disk space.
*			Jump table bug fixed (Assuming register to be 0!)
*			Small bug in PP decrunch routine killed.
*			Readme screen implemented with new gadget system.
*			Music play/readme flick bug fixed.
*
* -------------------------------------------------------------------------
...
* -------------------------------------------------------------------------
		...
		include	fission_chips_workdisk:powerpacker/include/libraries/powerpacker_lib.i
		include	fission_chips_workdisk:powerpacker/include/libraries/ppbase.i
...
GET_RESOURCES	jsr	_InitPlayer		; OctaMED call
		bne	CLOSE_PP		; 1 = Failure, quit.

LOAD_MUSIC	lea	module2,a0	; address of file
...
; a3 = address to unpack to??? (DOCS NEEDED)

LoadData	moveq	#DECR_POINTER,d0	; Decrunch type
		moveq	#MEMF_FAST,d1	
		lea	memory_p,a1		; Where to store pointer?
		lea	size,a2			; store size here....
		move.l	#0,a3			; Bug fix.
		CALLSYS	ppLoadData,_PPBase	; do it......		
	
; Music now in FAST mem. Let's make a buffer for it in CHIP...

		move.l	size,d0			; length of music
		move.l	#MEMF_CHIP,d1		; Chip memory
		CALLSYS	AllocMem,_SysBase	; grab it...	
		move.l	d0,music_p		; store pointer

; Now copy the data in PP buffer to our brand spanking new CHIP buffer:

		move.l	memory_p,a0		; source
		move.l	music_p,a1		; destination

		move.l	size,d0			; size

LoadDataLoop	move.b	(a0)+,(a1)+		; copy a byte
		subi.l	#1,d0			; sub 1 from count
		cmpi.l	#0,d0			; is it zero?
		bne	LoadDataLoop		; no - keep going...

; Now have copy in chip (where we want it) and in fast. Get rid of the
; fast copy.

		move.l	memory_p,a1		; address of memory
		move.l	size,d0			; size
		CALLSYS	FreeMem,_SysBase	; get rid of it.	
...
module1		dc.b	'Serendipity:mod.oppertune.pp',0
module2		dc.b	'Serendipity:mod.rellowmush.pp',0
module3		dc.b	'Serendipity:mod.septic child.pp',0
module4		dc.b	'Serendipity:mod.spacetrip3.pp',0
module5		dc.b	'Serendipity:mod.tangled in reins.pp',0
module6		dc.b	'Serendipity:mod.technic dreams.pp',0
		EVEN		
...
;seren_col	dc.w	$303,$EFF,$CEE,$ADD,$9CC,$7BB,$6AA,$599
...
ImageData1	incbin	"fission_chips_workdisk:pictures/serendipity.bm"
		EVEN		

ImageData2	incbin	"fission_chips_workdisk:pictures/readme_cd.bm"
		EVEN
